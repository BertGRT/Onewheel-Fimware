...
#include "mpu6050.h"
#include "balance.h"
// Variables observables par SWD (lecture live pendant les essais roues en l'air) :
volatile int16_t ow_pitch_x100 = 0;   // assiette x100 (deg)
volatile int16_t ow_state      = 0;   // etat machine (ow_state_t)
volatile int16_t ow_out        = 0;   // commande envoyee aux moteurs (-1000..1000)
volatile uint8_t ow_armed      = 1;   // SECURITE MAITRE (autonome : le footpad gere l'engagement ; GUI KILL = 0)
volatile int16_t ow_manual     = 0;   // TEST : si !=0, commande moteur directe (bypass MPU), SWD
extern volatile uint8_t ow_footpad;   // footpad logiciel (defini dans balance.c)
#endif
...
extern int16_t curDC_max;               // ONEWHEEL : protection courant DC (reglable live)
...
int main(void) {
  ...
  #ifdef VARIANT_ONEWHEEL
    balance_init();   // init MPU (PA2/PA3) + boucle d'equilibrage
  #endif
  ...
  #ifndef VARIANT_ONEWHEEL   // onewheel : pas de bouton (PA1 = mesure batterie) -> ne pas attendre
  ...
  while(1) {
    ...
    calcAvgSpeed();                       // Calculate average measured speed: speedAvg, speedAvgAbs
      ...
      #ifndef VARIANT_ONEWHEEL   // onewheel : l'enable est gere par la boucle d'equilibrage (ow_armed)
      ...
      #ifndef VARIANT_ONEWHEEL   // onewheel : pwml/pwmr ecrits UNIQUEMENT par le bloc balance
                                 // (sinon pwml=0 pendant la lecture MPU bloquante -> couple hache)
      ...
      // ############################ ONEWHEEL BALANCE ############################
      #ifdef VARIANT_ONEWHEEL
        // Limite de courant reglable en LIVE depuis la GUI (slider "Limite courant").
        //  - i_max  (courant phase, fixdt(1,16,4)) = le vrai plafond de couple.
        //  - curDC_max (protection courant DC, raw) suit avec +2A pour ne pas brider.
        // ow_config.current_limit est en A x10 (ex 200 = 20.0 A).
        rtP_Left.i_max = rtP_Right.i_max =
            (int16_t)(((int32_t)ow_config.current_limit * A2BIT_CONV * 16) / 10);
        curDC_max = (int16_t)(((int32_t)ow_config.current_limit * A2BIT_CONV) / 10
                              + 2 * A2BIT_CONV);

        int16_t man = ow_manual;
        if (man >  400) man =  400;        // borne de securite
        if (man < -400) man = -400;
        if (man != 0) {
          // ---- TEST MOTEUR DIRECT (bypass MPU/equilibrage) ----
          ow_out   = man;
          ow_state = -1;                   // marqueur "manuel"
        } else {
          // ---- Equilibrage normal ----
          balance_set_feedback(batVoltageCalib, left_dc_curr,
                               (int16_t)((rtY_Left.n_mot + rtY_Right.n_mot) / 2));
          ow_out        = balance_update();
          ow_pitch_x100 = (int16_t)(balance_telemetry()->pitch * 100.0f);
          ow_state      = balance_state();
        }
        if (!ow_armed) { ow_out = 0; }     // SECURITE MAITRE (GUI KILL = ow_armed=0)
        // enable moteurs : en manuel = si arme ; en equilibrage = seulement RIDING/TILTBACK
        // (pied absent -> IDLE -> moteurs LIBRES). ow_state=-1 (65535) en manuel.
        if (man != 0)
          enable = ow_armed;
        else
          enable = (ow_armed && (ow_state == OW_STATE_RIDING || ow_state == OW_STATE_TILTBACK)) ? 1 : 0;
        pwml = ow_out;                     // 2 moteurs meme sens (recablage) -> meme commande
        pwmr = ow_out;

        // ---- Sauvegarde flash de la config (bouton GUI) ----
        // L'ecriture flash GELE le CPU ~40ms : interdite en conduite.
        // Hors conduite (IDLE/ARME/FAULT) les moteurs sont deja a 0 -> sans danger.
        if (ow_save_req) {
          if (ow_state == OW_STATE_RIDING || ow_state == OW_STATE_TILTBACK) {
            ow_save_ack = 3;               // refuse : on roule
          } else {
            enable = 0; pwml = 0; pwmr = 0;
            ow_save_ack = ow_config_save();
          }
          ow_save_req = 0;
        }
      #endif
      // ######################## END OF ONEWHEEL BALANCE #########################
    #endif
